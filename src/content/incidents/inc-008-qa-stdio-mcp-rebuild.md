...
how: |
  <p>The user was explicit about the response: <em>"banned from dispatching another QA UNTIL YOU FIX THIS ISSUE … EVERYTHING NEEDS TO BE FIXED TOGETHER. NO DEFERRING WORK EVER."</em> The fix was an architectural rewrite, not a patch. The HTTP server was deleted. The new emission layer is a stdio MCP server at <code class="mono" style="font-size: 13px;">scripts/qa-emit-mcp.ts</code>: JSON-RPC over the <code class="mono" style="font-size: 13px;">claude -p</code> subprocess's stdin/stdout. Browser CSP does not apply because there is no browser request — the bytes move through process pipes, not network sockets.</p>
