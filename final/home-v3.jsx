// Homepage v3 — Single-claim hero with verbatim receipt.
// The hero IS a rule card. Numbers move below.
...
function HomeV3() {
  return (
    <div className="page" style={{ minHeight: "100%" }}>
      ...
      {/* Above-the-fold: one claim, one receipt */}
      <section style={{ padding: "56px 48px 0", borderBottom: "1px solid var(--rule)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 56, alignItems: "end" }}>
          <div>
            <div className="mono" style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--ink-4)", marginBottom: 24 }}>
            ...
            <h1 style={{
              ...
              Every claim on this site has a source.
            </h1>
            <p className="serif" style={{ fontSize: 19, color: "var(--ink-2)", maxWidth: "40ch", marginTop: 22, lineHeight: 1.5, marginBottom: 0 }}>
              Code Rescue does AI‑augmented engineering and due‑diligence audits for codebases that have to survive review. The methodology, the rule corpus, and the incident catalog on this site are the artifacts one of the engagements produces. The example below is a live entry — picked at random.
            </p>
          </div>
          <table className="data" style={{ fontSize: 13 }}>
            <tbody>
              <tr><td style={{ color: "var(--ink-4)" }}>Hours of AI‑augmented engineering across 30+ repos</td><td className="mono" style={{ textAlign: "right", color: "var(--ink)" }}>10,000+<FN n={1} /></td></tr>
              <tr><td style={{ color: "var(--ink-4)" }}>Production systems shipped</td><td className="mono" style={{ textAlign: "right" }}><a href="#engagements" style={{ color: "var(--accent)", borderBottom: "1px dashed var(--accent)" }}>6</a><FN n={2} /></td></tr>
              <tr><td style={{ color: "var(--ink-4)" }}>Hours on this methodology</td><td className="mono" style={{ textAlign: "right", color: "var(--ink)" }}>~2,000<FN n={3} /></td></tr>
              <tr><td style={{ color: "var(--ink-4)" }}>Major incidents catalogued</td><td className="mono" style={{ textAlign: "right" }}><a href="/incidents" style={{ color: "var(--accent)", borderBottom: "1px dashed var(--accent)" }}>9</a><FN n={4} /></td></tr>
              <tr><td style={{ color: "var(--ink-4)" }}>Enforcement rules in corpus</td><td className="mono" style={{ textAlign: "right" }}><a href="/rules" style={{ color: "var(--accent)", borderBottom: "1px dashed var(--accent)" }}>89</a><FN n={5} /></td></tr>
              <tr><td style={{ color: "var(--ink-4)" }}>QA flows in harness</td><td className="mono" style={{ textAlign: "right", color: "var(--ink)" }}>66<FN n={6} /></td></tr>
              <tr><td style={{ color: "var(--ink-4)" }}>Post‑deploy incidents on the six shipped products</td><td className="mono" style={{ textAlign: "right" }}><a href="/incidents" style={{ color: "var(--good)", borderBottom: "1px dashed var(--good)" }}>0</a><FN n={7} /></td></tr>
            </tbody>
          </table>
        </div>

        {/* The receipt — a real rule card pulled inline */}
        <div style={{ marginTop: 44, paddingTop: 32, paddingBottom: 36, borderTop: "1px solid var(--rule-2)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: 32, alignItems: "start" }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: "var(--ink-4)", fontFamily: "var(--mono)", marginTop: 8, lineHeight: 1.5 }}>
                A rule from the public corpus, with its origin incident.
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 0, border: "1px solid var(--rule)", background: "var(--paper-2)" }}>
              {/* LEFT side — the rule */}
              <div style={{ padding: "22px 26px", borderRight: "1px solid var(--rule)" }}>
                <div className="mono" style={{ fontSize: 10.5, color: "var(--ink-4)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>
                  ast‑grep · TypeScript · locked 2026‑02‑14
                </div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 10 }}>
                  <a href="#" style={{ fontSize: 17, fontWeight: 600, fontFamily: "var(--mono)", color: "var(--accent)", borderBottom: "1px dashed var(--accent)" }}>AG‑PO‑001</a>
                  <span className="pill" style={{ color: "var(--bad)", borderColor: "var(--bad)" }}>error</span>
                </div>
                <div className="serif" style={{ fontSize: 14.5, color: "var(--ink-2)", lineHeight: 1.55, marginBottom: 14 }}>
                  Property repository <code className="mono" style={{ fontSize: 13 }}>remove()</code> must use <code className="mono" style={{ fontSize: 13 }}>softDeleteSet()</code> — never <code className="mono" style={{ fontSize: 13 }}>db.delete()</code>.
                </div>
                <pre className="code" style={{ fontSize: 11.5, padding: "12px 14px", margin: 0, lineHeight: 1.5 }}>{`id: po-repo-no-hard-delete
language: typescript
rule:
  pattern: |
    db.delete($TABLE)
...
message: |
  ...
  Use softDeleteSet() — see DR‑PO‑014.`}</pre>
              </div>
              {/* RIGHT side — origin */}
              <div style={{ padding: "22px 26px", display: "flex", flexDirection: "column", gap: 18 }}>
                <div>
                  <div className="mono" style={{ fontSize: 10.5, color: "var(--ink-4)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Origin incident</div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 8 }}>
                    <a href="#" style={{ fontSize: 14, fontWeight: 600, fontFamily: "var(--mono)", color: "var(--accent)", borderBottom: "1px dashed var(--accent)" }}>INC‑026</a>
                    <span className="mono" style={{ fontSize: 11, color: "var(--ink-4)" }}>2026‑02‑14 · session 8a3f</span>
                  </div>
                  <div className="serif" style={{ fontSize: 13.5, color: "var(--ink-2)", lineHeight: 1.55 }}>
                    Agent regenerated <code className="mono" style={{ fontSize: 12 }}>removeProperty</code> using <code className="mono" style={{ fontSize: 12 }}>db.delete()</code> after a refactor; FK cascades took out 3 unrelated rows in dev. Caught in pre‑production by the QA harness.
                  </div>
                </div>
                <div>
                  <div className="mono" style={{ fontSize: 10.5, color: "var(--ink-4)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Cites</div>
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                    <a href="#" className="mono" style={{ fontSize: 12, color: "var(--accent)", borderBottom: "1px dashed var(--accent)" }}>DR‑PO‑014</a>
                    <a href="#" className="mono" style={{ fontSize: 12, color: "var(--accent)", borderBottom: "1px dashed var(--accent)" }}>P‑3</a>
                  </div>
                </div>
                <div style={{ marginTop: "auto", paddingTop: 14, borderTop: "1px solid var(--rule-2)" }}>
                  <a href="#" className="mono" style={{ fontSize: 11.5, color: "var(--ink-3)" }}>browse the full corpus → /rules</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="section tight" style={{ background: "var(--paper-2)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "180px 1fr 1fr 1fr", gap: 32 }}>
          ...
          {[
            { who: "PE / VC tech‑DD", what: "You're doing diligence on an AI‑augmented portfolio company. The methodology paper plus a redacted sample audit usually tell you whether the engagement is worth a call.", buy: "DRDD‑A" },
            { who: "CTOs at AI‑native SaaS", what: "You're preparing for a funding round, acquisition, or external audit. Install if the team will keep building. Audit if the codebase is a fixed artifact.", buy: "DRDD‑I" },
            { who: "Eng leaders rolling out AI", what: "You're rolling out AI‑augmented coding internally, and the rules you lock are going to outlive the rollout itself. Install, scoped to your rollout.", buy: "DRDD‑I" },
          ].map((b, i) => (
            <div key={i}>
      ...
      <section id="engagements" className="section tight">
        <div style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: 32, alignItems: "start" }}>
          <div>
      ...
      <section className="section tight" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, background: "var(--paper-2)" }}>
        <div>
      ...
      <Footnotes items={[
        <span>Logged via WakaTime (VS Code plugin) since 2023. ~12–14 hrs/day, 7 days/week, across 30+ client repositories.</span>,
